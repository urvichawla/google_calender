...
import { getCalendars as getCalendarsAPI, getSchedules as getSchedulesAPI } from '../../api';
...
export default function StoreProvider({ children }:
  ...
  useEffect(() => {
    ...
    if (isUser(user)) {
      Promise.all([
        getSchedulesAPI(user.uid),
        getCalendarsAPI(user.uid),
      ]).then(([fetchedSchedules, fetchedCalendars]) => {
        // Ensure we have arrays even if API returns null/undefined
        const schedules = Array.isArray(fetchedSchedules) ? fetchedSchedules : [];
        const calendars = Array.isArray(fetchedCalendars) ? fetchedCalendars : [];
        ...
        if (calendars.length === 0) {
          ...
          dispatchSchedules({
            type: StoreModel.UserAction.REPLACE_ALL,
            payload: schedules as Schedule[],
          })
          dispatchCalendars({
            type: StoreModel.UserAction.REPLACE_ALL,
            payload: calendars as Calendar[],
          })
          setStatus(prevStatus => ({
            ...prevStatus,
        ...
        // On error, initialize with local items
