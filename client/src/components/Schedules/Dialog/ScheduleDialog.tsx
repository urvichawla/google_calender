...
export default function ScheduleDialog(props: ScheduleDialogProps) {
	...
	const defaultScheduleState = {
		...
		calendarId: calendars && calendars.length > 0 ? calendars[0].id : uniqueID(),
		colorOption: calendars && calendars.length > 0 ? calendars[0].colorOption : getColorOption(),
		completed: false,
		dateTime: {
			allDay: false,
			once: true,
	...
	const { location, colorOption: _colorOption, ...taskProps } = scheduleProps;
	...
	useEffect(() => {
		if (calendars && calendars.length > 0 && !selectedSchedule) {
			setScheduleProps(prev => ({
				...prev,
				calendarId: calendars[0].id,
				colorOption: calendars[0].colorOption,
			}));
		}
	}, [calendars, selectedSchedule]);
